...
class ConvNext_conf():  # for all 32k sample rate models
    def __init__(self):
        ...
        self.hop_size = 320
        self.mel_bins =  224  #64 for cnn14 #224 for convnext
        self.fmin = 50
        self.fmax = 14000
        self.window = 'hann'
        self.center = True
...
parser.add_argument('-sr', type=int, default=32000) #COnvnext 32k
...
what_type = 'validate'
weak_csv = './data/flists/urban_sed_' + what_type +'_weak.tsv'
DF_weak = pd.read_csv(weak_csv, sep='\t',usecols=[0,1])  # only read first cols, allows to have messy csv

strong_csv = './data/flists/urban_sed_' + what_type +'_strong_modified.tsv'  #change path accordingly
...
def balanced_sample(choose_seq):
    ...
    ans = choose_seq[0]
    for i in range(1,len(choose_seq)):  # Inplace 1 you can use any number based how many neagative samples per mixture file
...
frames_num = 1001
num_freq_bin = 224  #64 for CNN14 and 224 for convNeXt
h5_name = './h5_files/' + what_type + '_cnn14_strong_plus.h5'  #change path accordingly
print(h5_name)
...
for i,filename in enumerate(weak_filename):
    ...
    strong_name = './audio/' + what_type + '/'+basename # validate/  #change path accordingly
    ...
    #print(negative_event_labels)
    for i in range(min(1,neg_len)):  #1 because we only need to pass 1 negative file to mixture file
