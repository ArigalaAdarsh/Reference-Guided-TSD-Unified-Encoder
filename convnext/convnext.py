...
from convnext.augmentations import (
    SpeedPerturbation,
    pydub_augment,
    roll_augment,
)
from convnext.pytorch_utils import do_mixup
from convnext.timm_weight_init import trunc_normal_
...
class ConvNeXt(nn.Module):
    ...
    def forward(self, x, mixup_lambda=None):
        ...
        x = x.unsqueeze(dim=1) # because we are already passing it the melspecgram
        ...
        #output_dict = {"clipwise_output": embedding, "clipwise_logits": logits }
    ...
    def forward_frame_embeddings(self, x, mixup_lambda=None):
        ...
        x = self.forward_features(x, return_frame_embeddings=True) # It return shape as  (B, emb, time, mels). we take mean along mels(dim=3)
        # print("after forward_features", x.size())
